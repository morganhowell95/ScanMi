...
public class DataEncryption {
	...
	// private HashSet<Pixel> thresh = new HashSet<Pixel>();
	...
	public Pixel encrypt(String message) {
				...
				// && (lettersPut < picCount)
				{
					...
					for (int i = x - Constants.MARGIN / 2; i < x
						...
						for (int j = y - Constants.MARGIN / 2; j < y
							...
							// newPix.addRed();
							// newPix.addGreen();
							// } else if (pixelsDrawn <= 12) {
							// newPix.addRed();
							// } else if(pixelsDrawn <= 13){
							//
							// newPix.addGreen();
							// }else if (pixelsDrawn <= 16) {
							//
							// newPix.addRed();
							// newPix.addGreen();
							// }

							// = p.offset(Constants.RGB_OFFSET);
					...
					// System.out.println(frame.getPixel(tempx,
					// tempy).getBrightness());
					// System.out.println(thresholdGrowth(tempx, tempy));
				}
	...
	public String search() {
		...
		while (iter.hasNext()) {
			...
			if (!previousP.rgbEquals(currentP) && !previousP.isWhite()
					&& !currentP.isWhite()) {
				// System.out.println(frame.getPixel(iter.getX(),
				// iter.getY()).getBrightness());
